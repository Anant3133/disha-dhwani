...
/**
 * Synthesizes speech using Coqui TTS for a given text.
 * Detects language automatically with franc and passes it to the Python script.
 *
 * @param {string} text - The input text to convert into speech.
 * @param {string} [language] - Optional language override; if not provided, detected automatically.
 * @returns {Promise<string>} - Resolves with the path to the generated audio file.
 */
async function generateSpeech(text, language = null) {
  ...
  if (!fs.existsSync(PYTHON_EXECUTABLE_IN_VENV)) {
    ...
    Ensure 'venv-coqui-311' is set up correctly and the path is accurate.`);
  ...
  // Escape quotes inside text and wrap all args in quotes
  ...
  return new Promise((resolve, reject) => {
    exec(command, execOptions, (error, stdout, stderr) => {
      if (error) {
    if (stderr && stderr.toLowerCase().includes("userwarning")) {
        console.warn(`[TTS Service Warning]: ${stderr}`);
    } else {
        console.error(`[TTS Service Error]: Coqui TTS exec error: ${error.message}`);
        console.error(`[TTS Service Error]: Coqui TTS stdout: ${stdout}`);
